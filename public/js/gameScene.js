class GameScene extends Phaser.Scene{
  ...
  create(){
    ...
    // Adjust scaling for responsive design
    ...
    this.ground_1 = this.ground.create(0, this.scale.height, 'ground').setScale(2).refreshBody();
    ...
    this.platform_1 = this.platforms.create(600, this.scale.height * 0.67, 'ground');
    this.platforms.create(50, this.scale.height * 0.42, 'ground');
    this.platforms.create(700, this.scale.height * 0.37, 'ground');
    ...
    this.health_bar = this.add.image(this.scale.width - 100, 30, this.health_imgs[this.health]).setScale(1.3);
    this.health_bar.setScrollFactor(0);
  }

  resize() {
    // Update the camera and world bounds
    ...
    if (this.health_bar) {
      this.health_bar.setScrollFactor(0);
      this.health_bar.x = this.scale.width - 100;
      this.health_bar.y = 30;
    }
    
    // Adjust game elements position if needed
    if (this.sky) {
      this.sky.setDisplaySize(this.scale.width, this.scale.height);
    }
  }
  ...
  update(){
    ...
    if (this.ground.getFirst(true).x <= this.physics.world.bounds.left) {
      this.ground.create(this.physics.world.bounds.left, this.scale.height, 'ground').setScale(4).refreshBody();
    }
    ...
    if (this.clock.now - this.old_time > 6000 / this.speed) {
      ...
      var platform_y = Phaser.Math.Between(100, this.scale.height - this.ground_1.height - this.platform_1.height - this.player.displayHeight - 150);
